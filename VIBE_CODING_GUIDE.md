...
## 1. 프로젝트 목적
- 펀드 판매대본(Excel/JSON)과 상품설명서(PDF)를 비교해 일치도와 불일치 근거를 생성한다.
- 결과는 사람이 검토 가능한 형태(JSON/테이블/로그)로 남긴다.

## 2. 핵심 디렉토리
- `agent/app.py`: Streamlit UI
- `agent/api_server.py`: FastAPI 서버, 요청 처리, 결과 저장, 일자별 로그 기록
- `agent/fund_core.py`: LLM 호출/파싱/비교 공통 로직
- `excel_json/excel_to_json.py`: Excel -> JSON 변환
- `data/output_excel_json`: 변환 산출물
- `data/output_agent`: 분석 결과(JSON)
...
## 4. 변경 시 체크리스트
- `pyproject.toml`과 `requirements.txt` 의존성을 항상 동기화한다.
- UI 변경 시 "일치도 분석 / 프롬프트 수정 / 데이터 / 로그" 메뉴 동작을 모두 점검한다.
- 로그/결과 파일 저장 경로가 README와 일치하는지 확인한다.
- 신규 환경변수 추가 시 `.env_example`과 README에 반영한다.

## 5. 실행/검증
- API 서버: `uvicorn agent.api_server:app --reload --port 8000`
- 웹 UI: `streamlit run agent/app.py`
- 테스트 스크립트: `python agent/api_server_test.py`
