...
def build_comparison_rows(result_json: dict) -> list:
    ...
    for key, value in result_json.items():
        ...
        if isinstance(value, list) and value:
            label = value[0]
            # if label in ("일치", "불일치"):
            script_text = str(value[1]) if len(value) > 1 else ""
            manual_text = str(value[2]) if len(value) > 2 else ""
            # 불일치일 때만 근거 표시, 일치이면 "일치합니다." 표시
            reason = str(value[3]) if len(value) > 3 else ""
            rows.append({
                "항목": key,
                "판정": label,
                "판매대본": script_text,
