...
def normalize_sheet_name(name: str) -> str:
    ...
    # 괄호/쉼표/기호 차이로 매칭이 깨지지 않도록 영숫자/한글만 남긴다.
...
def resolve_input_path(input_excel: str) -> Path:
    ...
    # NOTE:
    # - "~$..." 는 Excel 임시 잠금 파일 이름 패턴으로, 사용자 홈 경로가 아님.
    # - 일부 실행환경(예: 제한된 uv 런타임)에서는 expanduser()가 RuntimeError를 낼 수 있음.
    raw = Path(input_excel)
    if not input_excel.startswith("~$"):
        try:
            raw = raw.expanduser()
        except RuntimeError:
            # 홈 디렉토리를 확인할 수 없는 환경에서는 원문 경로를 그대로 사용한다.
            raw = Path(input_excel)
    if raw.exists():
        return raw

    # 1-1) 상대경로를 프로젝트 루트 기준으로도 해석 시도
    # 예: inspect_agent에서 ../../data/... 로 실행해도 data/...를 의도한 경우 구제
    if not raw.is_absolute():
        # (a) PROJECT_ROOT에 단순 결합한 경로
        candidate_project_relative = (PROJECT_ROOT / raw).resolve()
        if candidate_project_relative.exists():
            return candidate_project_relative

        # (b) 경로 내부에 data 세그먼트가 있으면 PROJECT_ROOT/data/... 로 재해석
        parts = list(raw.parts)
        if "data" in parts:
            data_idx = parts.index("data")
            candidate_from_data = (PROJECT_ROOT / Path(*parts[data_idx:])).resolve()
            if candidate_from_data.exists():
...
def main():
    ...
    parser.add_argument(
        "--output-dir",
        default=str(PROJECT_ROOT / "data/output_excel_json"),
        help="출력 폴더 경로",
    )
